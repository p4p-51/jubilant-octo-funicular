...
class ModuleService {
  getNextStage = async (moduleStage: IModuleStage): Promise<IModuleStage> => {
    const moduleStageCollection = await MongoAdapter.getInstance().getCollection(
      "moduleStage",
    );

    const stage = await moduleStageCollection.aggregate([
      {
    ...
    return stage[0]["nextStage"] ? stage[0]["nextStage"] : {module: "grad", stage:1};
  };
}
